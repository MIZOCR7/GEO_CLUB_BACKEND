...
def get_professor_response(user_input: str, history: List[Message]) -> str:
    ...
    **FOR EXPLANATIONS:**
    ...
    try:
        # Format messages for Gemini (concatenate system + history + current question)
        full_prompt = system_instruction + "\n\n--- CONVERSATION ---\n"
        
        # Add history
        for msg in history[-6:]:
            role = "Student" if msg.role in ["Student", "user"] else "Professor"
        ...
        if "content_filter" in error_msg or "safety" in error_msg or "safety rating" in error_msg:
            return "⚠️ Your message was flagged by our safety filter. Please try asking your question differently."
        elif "timeout" in error_msg or "deadline" in error_msg or "deadline exceeded" in error_msg:
            ...
            return "⚠️ System authentication error. Please check the API key in .env file."
